...
def build_chart(df: pd.DataFrame, chart_type: str, indices: List[str]) -> go.Figure:
    """
    ...
    if chart_type == 'Line Chart':
        return _build_line_chart(df, indices)
    elif chart_type == 'Area Chart':
        return _build_area_chart(df, indices)
    elif chart_type == 'Year-over-Year Change':
        return _build_yoy_chart(df, indices)
    else:
        raise ValueError(f"Unknown chart type: {chart_type}")


def _build_line_chart(df: pd.DataFrame, indices: List[str]) -> go.Figure:
...
def _build_area_chart(df: pd.DataFrame, indices: List[str]) -> go.Figure:
...
def _build_yoy_chart(df: pd.DataFrame, indices: List[str]) -> go.Figure:
